...
export function monthRate(ratePerHour, discount) {
  ...
  const discountedRate = applyDiscount(monthlyRate, discount);
...
export function daysInBudget(budget, ratePerHour, discount) {
  const discountedRate = applyDiscount(dayRate(ratePerHour), discount);
  return Math.floor(budget / discountedRate);
}
